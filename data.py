...
def read_texts(tarfname, dname):
    ...
    if dname == 'brown' and flag == 1:
        with open('reuters_sentences_to_append.pkl', 'rb') as f:
            ll = pickle.load(f)
            data.train += ll
#         with open('reuters_sentences_to_append.txt', 'r') as f:
#             reuters_val = f.readlines()
#         reuters_val = [x.strip() for x in reuters_val]
#         print(reuters_val[:10])
#         data.train += reuters_val
        
    #data.train[:] = data.train[:30000]
    data.test = []
...
def learn_trigram(data, verbose=True):
    ...
    if verbose:
        ...
        print("sample 1: ", " ".join(str(x) for x in sampler.sample_sentence(["my","dog"])))
        print("sample 2: ", " ".join(str(x) for x in sampler.sample_sentence(["good","morning"])))
    return trigram
